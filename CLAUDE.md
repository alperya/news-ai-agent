...
**Facebook Page cross-posting (whatever goes to Instagram goes to Facebook)**
When `FACEBOOK_PAGE_ID` is set, every Instagram publish is mirrored to the connected Facebook Page via `FacebookPublisher` (`src/social_publisher.py`): news/event Reels → **FB Reel** (`video_reels`), photo posts → **FB photo** (`/photos`), the daily fact Story → **FB Story** (`video_stories`). Each leg is **best-effort** — a Facebook failure logs + alerts but never blocks the Instagram publish. The Page access token is derived at runtime from the user token (`INSTAGRAM_ACCESS_TOKEN`), so nothing extra is stored; requires the `pages_manage_posts` scope. Cross-post call sites: `reels_worker` (news Reels), `_run_event_pipeline` (event Reels), the photo branch in `lambda_handler`, and the Story leg in `reels_worker`. Video flows (Reel/Story) follow Meta's two-phase upload: `start` → hosted `file_url` upload → poll status → `finish`.
