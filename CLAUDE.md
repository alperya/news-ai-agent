...
All prompts (`AI_PROMPT_BATCH_SELECTION`, `AI_PROMPT_SINGLE_ARTICLE`, `AI_PROMPT_QUALITY_CHECK`, `AI_PROMPT_EVENT_SELECTION`, `AI_PROMPT_CAROUSEL_CAPTION`) are stored in Secrets Manager alongside API keys. `get_secrets()` in `lambda_handler.py` loads them as env vars at startup. `ai_agent.py` reads from env, falls back to `prompts/` directory files (`prompts/*.txt` are gitignored; only `*.txt.example` are tracked, so production reads the Secrets Manager value). This allows hot-updating prompts without redeploying Lambda.
