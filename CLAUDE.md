...
Whatever is published to Instagram is mirrored to every other configured channel via a single dispatcher. `ChannelPublisher` is the common interface (`name`, `supports(kind)`, `publish(kind, media_url, caption, dry_run)`); `kind ∈ {REEL, PHOTO, STORY}`. Concrete adapters live in `src/social_publisher.py`: `InstagramPublisher` (primary) and `FacebookPublisher` (secondary). `CrossPoster.publish(kind, …)` fans out to the primary (failure surfaced via `primary_error`) + every secondary that `supports(kind)` **best-effort** (logged + alerted, never blocks); it returns `{"results": {channel: result}, "primary_error": Exception|None}`. `build_crossposter()` registers the primary plus any secondary whose creds are present (`FACEBOOK_PAGE_ID` → Facebook). **Adding a channel = write one `ChannelPublisher` adapter + one line in `build_crossposter()`; call sites never change.** Call sites: `reels_worker` (Reel + Story), `_run_event_pipeline` (event Reel), photo branch in `lambda_handler`. Facebook: news/event Reels → **FB Reel** (`video_reels`), photos → **FB photo** (`/photos`), daily Story → **FB Story** (`video_stories`); Page token derived at runtime from the user token (`pages_manage_posts` scope). Video flows follow Meta's two-phase upload: `start` → hosted `file_url` upload → poll status → `finish`. **When new channel SDKs push the Lambda ZIP past 250 MB, migrate to a Lambda Container Image (ECR) — not microservices.**
...
| `src/social_publisher.py` | Channel adapters: `InstagramPublisher` (Graph API v24.0 — photo/Reels/Story) + `FacebookPublisher` (Page Reels/photos/Stories) |
