...
- ✅ Tiered news selection (🇳🇱 Holland → 🇪🇺 Europe → 🌍 Global) ranked on **two equal axes**: a SAVE score (does it cost you money, change your commute, threaten your street, set a date?) and a SHARE score (is it a record or anomaly, a nationally recognised name, something people want to react to, a "look at this" frame). Stories rank on the sum; one scoring zero on both is never picked. Reach on this account tracks shares at Spearman 0.86, so a save-only ranking quietly removed every breakout post
- ✅ **Content saturation caps** — no single topic may exceed 30% of the rolling 7-day mix and no named series may run more than 4 consecutive days, alongside the existing 40% crime cap. Stated in the prompt, never hard-filtered, so a genuine national emergency can still override
...
4. **Geographic footage safety** — stock footage never claims a place we can't source, and never stops looking Dutch either. Queries for small-town stories are stripped of place names (Pexels always returns *something* — asking for a small Dutch town returns a different town) but are guaranteed to keep at least two `dutch`/`netherlands` anchors, since a country is a promise a town name never is. Candidate clips are dropped for free when their URL slug names another place, frames an unnamed city, or shows terrain the Netherlands does not have (mountains, gorges, cliffs, tropics). A single Opus vision gate then applies four tests to every clip — does it show the story's **subject** (no fireplaces for a wildfire), does it claim an unverifiable place, could it be the Netherlands, and does it hit the story's avoid list — while generically-Dutch wide shots (polders, canals, motorways) explicitly pass. Place-specific stories give the article's own photo a longer cover + a mid-Reel reprise, stock segments carry a "STOCK FOOTAGE · ILLUSTRATION" label, and every accept/reject decision is persisted (`footage_plan`/`footage_audit` in `posts_*.json`)
